                    ...
                    <div class="border rounded-lg p-4 shadow-sm {{ $loop->odd ? 'bg-gray-50' : '' }}">
                        <div class="flex justify-between items-center mb-2">
                            <span class="font-semibold text-gray-800">
                                {{-- ============ INICIO DEL CAMBIO ============ --}}
                                @php
                                    // 1. Determina qué fecha usar (la de la cita o la de creación del historial)
                                    $fecha = $entrada->cita?->fecha_hora_inicio ?? $entrada->created_at;
                                    
                                    // 2. Formatea la fecha si existe, la pone en español, y capitaliza el resultado
                                    //    'isoFormat' es más fiable para traducciones que 'translatedFormat'
                                    //    'ucfirst' capitaliza la primera letra (ej: "sábado" -> "Sábado")
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-200">
                            ...
                            <div class="mt-2">
                                <button wire:click="descargarResultado({{ $entrada->cita->ordenLaboratorio->id }})" 
                                        ...
                                        wire:target="descargarResultado({{ $entrada->cita->ordenLaboratorio->id }})"
